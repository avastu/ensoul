...
The user can set a standing **dial** — `plain`, `balanced`, or `wild` — usually chosen once at install and recorded in the agent's own memory/instructions. It shifts the default along one axis: how much plain text versus visual wildness/abstraction they want. `plain` leans on words with a few marks; `balanced` is the small glanceable visual described here (and the default when the dial is unset); `wild` permits looser, stranger, more abstract artifacts by default. Honor the dial when it is set; a per-invocation request ("go wild," "just words") overrides it for that turn. Whatever the dial, the no-lie / no-dazzle gates still hold and bare invocations stay bounded.
...
**Configure from the user's own material, not abstract description.** At install (or first run), take a real recent artifact from their environment — a transcript, a diff, the project's state, a file they just touched — and render it at all three dial settings: `plain`, `balanced`, `wild`. Let them pick the dial by seeing Ensoul on their own work, then capture the dial and the mode and record both in the agent's memory/instructions. Showing it on their material is the demo and the configuration in one move; never make them choose `plain`/`balanced`/`wild` from words alone.
...
When the user explicitly asks for weirder, wilder, Rick-Rubin-like, Ruth-Asawa-like, jazz-like, appalling, wrong, risky, or permission-to-fail artifacts, loosen the usual politeness and produce a stranger designed object.
